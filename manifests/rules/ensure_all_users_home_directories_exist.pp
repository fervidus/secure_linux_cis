...
class secure_linux_cis::ensure_all_users_home_directories_exist (
  ...
  if $enforced {
    ...
    if !($facts['home_directory'].empty) {
      notify { 'hdir':
        message  => 'Not in compliance with CIS  (Scored). You have a user(s) that does not have a home directory. Check the home_directory fact for details',#lint:ignore:140chars
        loglevel => 'warning',
      }
    }
  }
